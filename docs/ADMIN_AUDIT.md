...
**Осталось (P1):**
- ⬜ P1-13 мультиязычный редактор объекта (`property_translations`/`slug_localized`) — крупный (делает владелец)
- 🟡 P1-14 (хвост) — ✅ редактируемые колонки футера (`dbb482d`); осталось: вложенность/дропдауны навигации (`parent_id`), выбор `page_id` (дизайн-чувствительно)
...
- ✅ Bookings: поиск + фильтр источника + пагинация + колонка Source + диапазон дат (`95fa495`, `99aae31`)
- ✅ редактируемые колонки футера Browse/Areas/Legal (white-label, фикс Дубай-хардкода) (`dbb482d`)
- ✅ редактируемый header-подзаголовок «Licensed Realtor» (white-label) (`966851a`)
- ✅ CRM Activity timeline (лид/сделка/контакт) + логирование CRM-мутаций в audit_logs (`4b1488e`)
- ✅ активация `lead_sources`: фильтр по источнику + аналитика «leads by source» (`d6667d0`)
- ✅ профиль агента: таблица `agent_profiles` + редактор + публичный показ + «Listed by» (`f3b8602`, `26056a4`)
- ✅ нижняя строка футера (языки/валюты из настроек организации) (`068a3bf`)
- ✅ open/click email-трекинг через Resend-вебхуки → метрики кампаний (`ece62d1`)

- ✅ авто-рефреш сегментов рассылки перед отправкой (`c3e1863`)

**Осталось (P2):** множественные цены объекта, документы/видео в Storage, карта в локации
объекта (всё — в большой property-form, риск), `syncReports`/Meta CAPI (нужны внешние ключи),
серверный PDF-инвойс.
...
- ✅ Open/click-трекинг писем через Resend-вебхуки (`delivered`/`opened`/`clicked` → `campaign_reports`) (`ece62d1`); требует включения open/click tracking в Resend. [6]
...
- ✅ Сущность агента: таблица `agent_profiles` (bio, RERA, phone, специализация, фото, title) + self-service редактор в Settings; показ на `/agents`, `/agents/[id]` и ссылка из «Listed by» (`f3b8602`, `26056a4`). [5]
...
- ✅ Вынести «обвязку» в редактируемые поля: монограмма (`3ed639c`), «Licensed Realtor» (`966851a`), районы Дубая в футере (`dbb482d`), нижняя строка футера языки/валюты (`068a3bf`). [5]
...
- ✅ Авто-рефреш сегментов рассылки перед отправкой (`c3e1863`). [6]
- ✅ Задействовать `lead_sources` + аналитика по источникам (`d6667d0`). [4]
- ✅ Лента активности (timeline) на лиде/контакте/сделке из `audit_logs` (`4b1488e`). [4]
