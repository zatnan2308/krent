...
**Осталось (P2):** множественные цены объекта, документы/видео в Storage, профиль агента
(новая таблица), карта в локации объекта, open/click email-трекинг, авто-рефреш сегментов,
нижняя строка футера (языки/валюты), `syncReports`/Meta CAPI (нужны внешние ключи), серверный
PDF-инвойс.
