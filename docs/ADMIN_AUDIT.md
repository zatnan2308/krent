...
**Осталось (P2)** — каждый требует режима, который нельзя сделать «вслепую» без визуальной
проверки или внешних ключей:
- документы/видео объекта в Storage — механика загрузки (лимиты server actions на видео,
  политики bucket) не проверяема без запуска;
- карта в локации объекта — нужен внешний map/geocoding API и ключ;
- `syncReports` (GSC/Google Ads/Meta), Meta CAPI — нужны внешние ключи/токены пользователя;
