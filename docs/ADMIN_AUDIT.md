...
- ✅ P1-14 ЗАКРЫТ: колонки футера (`dbb482d`) + дропдауны навигации (`parent_id`) + выбор `page_id` (`be06ef1`) ⚠️ нужна визуальная проверка дропдаунов
...
- ✅ превью кастомных CMS-страниц (список + редактор) (`b8534b4`)
- ✅ множественные цены объекта (mixed sale+rent): форма + публичный показ + booking (`93ecb37`) ⚠️ нужна твоя визуальная проверка
- ✅ дропдауны навигации (`parent_id`) + привязка пункта к странице (`page_id`) (`be06ef1`) ⚠️ нужна твоя визуальная проверка

**Осталось (P2)** — каждый требует ТВОЕГО участия (внешние ключи / визуальная проверка):
- **карта в локации объекта** — Leaflet/OSM можно без ключа, но виджет в форме с asset/CSS/SSR-
  гочами почти наверняка сломается «вслепую» → нужна визуальная проверка (lat/lng руками — работает);
- **`syncReports`** (GSC/Google Ads/Meta), **Meta CAPI** — нужны твои внешние ключи/токены;
- **видео объекта в Storage** — большие файлы не лезут в лимит upload server actions (URL — работает);
- свести два CMS-контура — крупно, пересекается с P1-13 (мультиязык — делает владелец).

**✅ Закрыто из «осталось»:**
- серверный PDF-инвойс через `pdf-lib` (`893c3d4`) — рантайм-проверен (валидный PDF, не падает на не-латинских именах); инвойс теперь залинкован из деталей брони.
- документы объекта в Storage (`fb8e67f`) — загрузка PDF/image/Word ≤10MB (переиспользует пайплайн медиа); видео остаётся по URL.
...
- ✅ Превью в контент-редакторах: home/about (`f181ac0`), объекты (`41e4dd7`), кастомные страницы (`b8534b4`) — «открыть в новой вкладке». [5]
