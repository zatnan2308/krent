...
Vercel Cron вызывает endpoint раз в час. На endpoint должен быть установлен
secret-header (`CRON_SECRET`), который проверяется в route handler'е.
